...
./.venv/bin/python crawler/run_batch.py \
  --udid <UDID> --gu 영등포구 --dongs 문래동 영등포동
```

`--dongs`에 넘긴 동을 순서대로 전환해가며 `KEYWORDS`의 모든 키워드를 검색한다.
**단, 동 전환은 그 폰에 이미 "내 동네"로 등록(인증)된 동만 가능하다** — 당근 앱은
최대 2개 동네까지만 등록되고, 새 동네를 추가하려면 GPS로 그 동 근처에 있어야
인증되기 때문에(실기기 확인) 스크립트가 가보지 않은 동을 자동으로 등록할 수는
없다. 등록 안 된 동을 만나면 그 지점에서 멈추고 안내 문구를 출력한다 — 폰에서
그 동으로 가서(또는 위치를 옮겨서) "내 동네 설정"으로 먼저 등록한 뒤 같은 명령을
다시 실행하면, 이미 처리한 동·키워드는 dedup 키 덕분에 건너뛰고 이어서 처리된다.
